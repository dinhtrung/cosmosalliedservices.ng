<?php if ( ! function_exists( 'kindergarten_comment' ) ) :

function kindergarten_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<header class="clearfix">
                ...
                <?php printf( __( '%s', 'kindergarten' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
                </div><!-- .comment-author .vcard -->
                
				<?php if ( $comment->comment_approved == '0' ) : ?>
                	<div class="waiting-moderation">
					<em><?php esc_html_e( 'Your comment is awaiting moderation.', 'kindergarten' ); ?></em>
					</div>
				<?php endif; ?>
				...
				said on
					...
					<?php printf( _x( '%1$s', '1: date', 'kindergarten' ), get_comment_date()); ?>
					</time>
				</div><!-- .comment-meta .commentmetadata -->
               

                <div class="comment-action">
            	<span class="edit-comment-link"><?php edit_comment_link( __( 'Edit', 'kindergarten' ), '<span class="edit-link">', '<span>' ); ?></span>
...
function kindergarten_comment_form(){
